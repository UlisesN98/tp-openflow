...
\begin{itemize}
    ...
    \item \textbf{OpenFlow 1.0:} La versión del protocolo utilizada tiene capacidades limitadas:
    \begin{itemize}
        \item No soporta nativamente \texttt{IPv6}, matching de campos arbitrarios, ni metadata
        \item Solo reconoce oficialmente TCP y UDP para matching de puertos (\texttt{tp\_src}/\texttt{tp\_dst})
        \item SCTP puede especificarse como protocolo (\texttt{nw\_proto=132}) pero sin soporte de puertos
        \item Versiones posteriores (1.3+)\cite{openflow13} ofrecen mayor flexibilidad y soporte extendido de protocolos
    \end{itemize}
\end{itemize}
