...
\textbf{Objetivo:} Verificar que el puerto 80 está bloqueado para todos los protocolos de transporte (\texttt{TCP} y \texttt{UDP}).
...
\textbf{Análisis:}
\begin{itemize}
    \item  Se observan 5 reglas instaladas con prioridad 10000 (firewall)
    \item  Las reglas de puerto 80 incluyen solo \texttt{TCP} y \texttt{UDP} (SCTP excluido por limitaciones de OF 1.0)
    \item  Las reglas incluyen matching por protocolo (\texttt{tcp/udp}), puerto (80, 5001), y direcciones \texttt{MAC}
