...
\begin{enumerate}
    ...
    \item \textbf{Pruebas de rendimiento}: Medición de throughput y latencia con iperf
...
\begin{itemize}
    ...
    \item Sistema operativo: Linux (Ubuntu/Debian)
...
\textbf{Análisis:} La comunicación entre h2 y h4 funciona correctamente, confirmando que el sistema permite tráfico no restringido.

\subsubsection{Test 2: Bloqueo de Puerto 80 (TCP y UDP)}

\textbf{Objetivo:} Verificar que el puerto 80 está bloqueado para todos los protocolos de transporte (TCP, UDP, SCTP).
...
\paragraph{Test 2.1: Bloqueo TCP Puerto 80}

\textbf{Procedimiento:}
\begin{enumerate}
    \item Iniciar servidor HTTP en h3 puerto 80
    \item Intentar conexión desde h2 usando curl
\end{enumerate}
...
\textbf{Análisis:} El tráfico TCP al puerto 80 está correctamente bloqueado.

\paragraph{Test 2.2: Bloqueo UDP Puerto 80}

\textbf{Procedimiento:}
\begin{enumerate}
    \item Iniciar listener UDP en h3 puerto 80
    \item Enviar datos desde h2 usando netcat
\end{enumerate}
...
\textbf{Resultado esperado:} El mensaje no debe llegar a h3

\textbf{Resultado obtenido:} El servidor en h3 no recibe ningún dato.

\textbf{Análisis:} El tráfico UDP al puerto 80 está correctamente bloqueado.

\subsubsection{Test 3: Bloqueo Específico h1 $\rightarrow$ UDP:5001}

\textbf{Objetivo:} Verificar que solo el host h1 está bloqueado para enviar tráfico UDP al puerto 5001, mientras que otros hosts pueden hacerlo.
...
\paragraph{Test 3.1: h1 $\rightarrow$ h3:5001 (Debe Fallar)}
...
\textbf{Resultado obtenido:} El servidor en h3 NO recibe el mensaje.

\textbf{Análisis:} El tráfico UDP desde 10.0.0.1 al puerto 5001 está bloqueado.

\paragraph{Test 3.2: h2 $\rightarrow$ h3:5001 (Debe Funcionar)}
...
\textbf{Análisis:} El tráfico UDP desde otros hosts (h2) al puerto 5001 está permitido, confirmando la granularidad de la regla.

\subsubsection{Test 4: Bloqueo Bidireccional h1 $\leftrightarrow$ h3}

\textbf{Objetivo:} Verificar que el tráfico entre h1 y h3 está bloqueado en ambas direcciones, independientemente del protocolo.
...
\paragraph{Test 4.1: h1 $\rightarrow$ h3 (Debe Fallar)}
...
\textbf{Análisis:} Tráfico de h1 a h3 bloqueado correctamente.

\paragraph{Test 4.2: h3 $\rightarrow$ h1 (Debe Fallar)}
...
\paragraph{Test 4.3: h1 $\rightarrow$ h2 (Debe Funcionar)}

\textbf{Objetivo:} Verificar que h1 puede comunicarse con otros hosts no bloqueados.
...
\textbf{Análisis:} h1 puede comunicarse con hosts no restringidos, confirmando que la regla es específica para h1 $\leftrightarrow$ h3.
...
\textbf{Conclusión:} Todas las pruebas funcionales (9/9) pasaron exitosamente, demostrando que el firewall SDN implementa correctamente las políticas de seguridad definidas.

\subsection{Pruebas de Rendimiento con iperf}

Se utilizó la herramienta \texttt{iperf} para medir el throughput y evaluar el impacto del firewall en el rendimiento de la red. Las pruebas se ejecutaron entre diferentes pares de hosts con y sin reglas de firewall aplicadas.
...
\bottomrule
\end{tabular}
\caption{Resultados Test 5: TCP h2 $\rightarrow$ h4 (sin firewall en el camino)}
...
\textbf{Configuración:}
\begin{itemize}
    \item Servidor: h3 (puerto 5201)
    \item Cliente: h2
    \item Duración: 10 segundos
    \item Protocolo: TCP
    \item Nota: El switch s2 (firewall) está en el camino, pero el puerto 5201 NO está bloqueado
\end{itemize}
...
\bottomrule
\end{tabular}
\caption{Resultados Test 6: TCP h2 $\rightarrow$ h3 (con firewall activo)}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Posible diferencia en rutas de red (h4 vs h3 en topología)
...
\bottomrule
\end{tabular}
\caption{Resultados Test 7: UDP h2 $\rightarrow$ h4}
\label{tab:test7_results}
\end{table}

\textbf{Análisis:}
\begin{itemize}
    \item  Throughput alcanzado: 105 Mbps (objetivo: 100 Mbps) - ligeramente superior debido al overhead de UDP
    ...
    \item  Conclusión: La red virtualizada y el firewall SDN manejan tráfico UDP sin degradación
\end{itemize}

\subsubsection{Test 8: Verificación de Bloqueo con iperf (Puerto 80 TCP)}

\textbf{Objetivo:} Confirmar que el firewall bloquea correctamente el tráfico en puerto restringido usando iperf.

\textbf{Configuración:}
\begin{itemize}
    \item Servidor: h3 (puerto 80 - \textbf{BLOQUEADO})
    \item Cliente: h2
    \item Protocolo: TCP
    \item Regla activa: Block port 80 (TCP)
\end{itemize}
...
\begin{itemize}
    ...
    \item  El firewall descartó los paquetes SYN del TCP handshake
    \item  Esto confirma que la regla de bloqueo del puerto 80 TCP funciona correctamente
    \item  Comportamiento esperado: timeout en lugar de ``Connection refused" porque los paquetes son silenciosamente descartados (DROP) en lugar de rechazados con RST
\end{itemize}
...
\textbf{Objetivo:} Medir la latencia introducida por el controlador SDN en el camino de datos.
...
\bottomrule
\end{tabular}
\caption{Estadísticas de latencia ICMP (h2 $\rightarrow$ h4)}
...
\begin{enumerate}
    ...
    \item \textbf{TTL = 54:}
    \begin{itemize}
        \item Indica que los paquetes atravesaron múltiples switches (TTL inicial típicamente 64)
...
\begin{itemize}
    ...
    \item  Las reglas incluyen matching por protocolo (tcp/udp/sctp), puerto (80, 5001), y direcciones MAC
    \item  Todas las reglas tienen acción ``drop''
...
\begin{enumerate}
    \item \textbf{Corrección funcional:} Todas las reglas de firewall se aplican correctamente, bloqueando tráfico específico por protocolo, puerto, IP y MAC.
...
\begin{itemize}
    ...
    \item Throughput ligeramente menor cuando el tráfico atraviesa el switch de firewall (s2)
    \item Dependencia de un controlador centralizado (single point of failure)
\end{itemize}

\textbf{Casos de uso validados:}
\begin{itemize}
    \item  Bloqueo de servicios (HTTP puerto 80)
    \item  Aislamiento de hosts problemáticos (h1 $\leftrightarrow$ h3)
    \item  Filtrado selectivo por IP y puerto (h1 $\rightarrow$ UDP:5001)
    \item  Políticas genéricas multi-protocolo (puerto 80 en TCP/UDP/SCTP)
\end{itemize}