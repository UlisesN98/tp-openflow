...
\hline
Protocolo de descubrimiento & Protocolo de Spanning Tree (STP) para evitar loops & Protocolos de enrutamiento (RIP, OSPF, BGP) \\
\hline
...
\begin{itemize}
    \item \textbf{Switch:} Si el host \texttt{h1} (MAC: \texttt{00:00:00:00:00:01}) envía un frame a \texttt{h3} (MAC: \texttt{00:00:00:00:00:03}), el switch consulta su tabla MAC y reenvía el frame únicamente por el puerto donde aprendió que está \texttt{h3}. Si no conoce la MAC destino, hace flooding.
    
    \item \textbf{Router:} Si el host \texttt{192.168.1.10} envía un paquete a \texttt{10.0.0.5}, el router consulta su tabla de rutas, determina que debe salir por la interfaz conectada a la red \texttt{10.0.0.0/24}, decrementa el TTL, recalcula el checksum IP, y reencapsula el paquete en un nuevo frame Ethernet con la MAC del siguiente salto.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Función de forwarding:} Ambos toman decisiones sobre cómo reenviar tráfico según información en sus tablas (MAC-to-port para switches, tabla de rutas para routers).
    ...
    \begin{itemize}
        ...
        \item ASICs o procesadores especializados para procesamiento de paquetes
        \item Bus interno de alta velocidad para comunicación entre puertos
    \end{itemize}
    
    \item \textbf{Paradigma store-and-forward:} Ambos reciben el paquete completo, lo almacenan temporalmente, verifican su integridad (CRC/checksum), y luego lo reenvían.
    
    \item \textbf{Separación plano de control y plano de datos:}
    \begin{itemize}
        \item \textbf{Plano de control:} Construye y mantiene las tablas (aprendizaje MAC para switches, protocolos de enrutamiento para routers)
        \item \textbf{Plano de datos:} Utiliza las tablas para reenviar paquetes a alta velocidad
    \end{itemize}
    
    \item \textbf{Soporte para VLANs (en switches modernos):} Los switches pueden segmentar redes lógicamente, y algunos routers incorporan funcionalidad de switching L2.
    
    \item \textbf{Gestión y configuración:} Ambos suelen ofrecer interfaces de gestión (CLI, SNMP, web) para configuración y monitoreo.
\end{enumerate}
...
\begin{itemize}
    ...
    \item Incluyen capacidades de enrutamiento IP (L3)
    \item Optimizados para enrutamiento dentro del mismo dominio administrativo (intra-AS)
    \item Ejemplo: Cisco Catalyst 3850, Arista 7050
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Decisión de forwarding:} Al recibir un frame con MAC\_dst = M:
    \begin{itemize}
        \item Si M está en la tabla: reenviar por puerto asociado
        \item Si M no está: flooding (enviar por todos los puertos excepto el de entrada)
        \item Si M es broadcast (\texttt{ff:ff:ff:ff:ff:ff}): flooding
    \end{itemize}
    
    \item \textbf{Protocolos embebidos:} Spanning Tree Protocol (STP) para prevenir loops, VLAN Trunking Protocol (VTP) para sincronizar VLANs, etc.
...
\begin{enumerate}
    ...
    \item \textbf{Flow Table:} En lugar de una simple tabla MAC-to-port, los switches OpenFlow mantienen una \textbf{tabla de flujos} con entradas de la forma:
    \begin{verbatim}
    ...
    \begin{itemize}
        ...
        \item \texttt{modify\_field} - Modificar campos (MAC, IP, VLAN)
    \end{itemize}
...
\begin{enumerate}
    \item \textbf{Border Gateway Protocol (BGP):} Protocolo estándar para intercambiar información de enrutamiento entre ASes diferentes. Cada AS anuncia sus prefijos IP y aprende rutas hacia otros ASes.
    ...
    \begin{itemize}
        ...
        \item Seguridad (filtrado de rutas, RPKI)
    \end{itemize}
    ...
    \item \textbf{Escalabilidad:} La tabla de rutas BGP global contiene >900,000 prefijos IPv4 y >150,000 prefijos IPv6 (datos 2024).
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Single point of failure:} Un controlador centralizado para Internet sería un objetivo crítico para ataques DDoS y fallos catastróficos.
\end{itemize}
...
\textbf{Problema:} Los switches OpenFlow tienen capacidad limitada en sus flow tables (típicamente 1,000 - 10,000 entradas en TCAM).

\begin{itemize}
    \item \textbf{Tabla de rutas global:} >900,000 prefijos IPv4 no caben en la TCAM de un switch OpenFlow estándar.
...
\begin{itemize}
    ...
    \item \textbf{Multipath routing:} BGP soporta ECMP (Equal-Cost Multi-Path). OpenFlow 1.0 tiene soporte limitado para balanceo de carga entre múltiples caminos.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Loops de routing:} BGP usa AS-PATH para prevenir loops. OpenFlow no tiene un mecanismo análogo incorporado.
\end{itemize}
...
\textbf{Ejemplo:} Este trabajo implementa un firewall SDN en una topología de 4 switches. Escalar a 100-1000 switches en un data center es factible con controladores de alta disponibilidad (ej: ONOS con clustering).
...
\textbf{Ejemplo real:} Google B4 WAN
...
\paragraph{3. Segment Routing (SR) y SRv6}\mbox{}\\
...
\paragraph{4. P4 y Programmable Data Planes}\mbox{}\\
