...
El sistema implementa una arquitectura SDN basada en el paradigma de separación entre el plano de control y el plano de datos. El controlador POX, ejecutándose en \texttt{Python}, actúa como cerebro centralizado que gestiona el comportamiento de los switches mediante el protocolo OpenFlow 1.0.

La arquitectura sigue el modelo clásico de SDN de tres capas:

\begin{enumerate}
    \item \textbf{Capa de Aplicación:} Políticas de firewall definidas en formato \texttt{JSON} que especifican qué tráfico debe ser bloqueado.
    ...
    \item \textbf{Plano de Datos:} \textbf{Switches Open vSwitch}\cite{openvswitch} que ejecutan las reglas instaladas y reenvían paquetes según la tabla de flujos.
\end{enumerate}
...
La topología se implementa mediante el script \texttt{topology.py} utilizando la \texttt{API} de Mininet. Cada switch tiene un \texttt{DPID} (DataPath ID) único, siendo el switch \texttt{s2} (\texttt{DPID 00-00-00-00-00-02}) el designado para aplicar las reglas de firewall.
...
\begin{itemize}
    \item \textbf{Match Fields:} Especifican los criterios de matching (direcciones \texttt{MAC/}IP, protocolos, puertos).
    \item \textbf{Priority:} Define la precedencia de la regla. El sistema usa prioridad 10000 para firewall y 100 para forwarding.
    \item \textbf{Actions:} Indican qué hacer con los paquetes que matchean. Para firewall, la lista de acciones está vacía (\texttt{DROP} implícito).
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{get\_rules()}: Carga y expande reglas desde \texttt{JSON}
    ...
    \item \texttt{rules.json}: Archivo de configuración con políticas de firewall en formato \texttt{JSON}. Define reglas de bloqueo por \texttt{MAC}, \texttt{IP,} protocolo y puerto.
...
Las políticas de seguridad se definen mediante un archivo \texttt{JSON}\cite{json_format} con la siguiente estructura:

\begin{lstlisting}[language=json, caption={Ejemplo de archivo \texttt{rules.json}}, label={lst:rules_json}]
[
    {
        "name": "Block port 80",
        "protocol": "ANY",
...
\begin{description}
    \item[\texttt{name}] (\texttt{string}): Descripción legible de la regla
    \item[\texttt{protocol}] (\texttt{string}): Protocolo de transporte/red (``\texttt{TCP}", ``\texttt{UDP}", ``\texttt{ICMP}", ``\texttt{SCTP}", ``\texttt{ANY}"). 
    \begin{itemize}
        \item \texttt{TCP/UDP}: Soporte completo de puertos (\texttt{tp\_src/tp\_dst})
        \item \texttt{ICMP}: Sin puertos (matching solo por protocolo)\footnote{Si bien \texttt{ICMP} no es como tal un protocolo de transporte, se lo pone acá, ya que es la forma que tiene OpenFlow y POX para distingirlo con \texttt{nw\_proto}}
        \item \texttt{SCTP}: Soporte básico de protocolo sin matching de puertos en OpenFlow 1.0
        \item \texttt{ANY}: Se expande solo a TCP y UDP cuando se especifican puertos
    \end{itemize}
    \item[\texttt{src\_mac}] (\texttt{string}): Dirección \texttt{MAC} origen (formato XX:XX:XX:XX:XX:XX)
    \item[\texttt{dst\_mac}] (\texttt{string}): Dirección \texttt{MAC} destino
    \item[\texttt{src\_ip}] (\texttt{string}): Dirección \texttt{IPv4} origen
    \item[\texttt{dst\_ip}] (\texttt{string}): Dirección \texttt{IPv4} destino
    \item[\texttt{mask\_src}] (\texttt{int}): Máscara de subred para \texttt{IP} origen (CIDR)\cite{aws_cidr}
    \item[\texttt{mask\_dst}] (\texttt{int}): Máscara de subred para \texttt{IP} destino (CIDR)
    \item[\texttt{src\_port}] (\texttt{int}): Puerto origen (para \texttt{TCP/UDP/SCTP})
    \item[\texttt{dst\_port}] (\texttt{int}): Puerto destino (para \texttt{TCP/UDP/SCTP})
\end{description}

\subsubsection{Expansión de Reglas Genéricas}

Las reglas con \texttt{protocol:} ``\texttt{ANY}" que especifican puertos se expanden automáticamente en múltiples reglas específicas:
...
Ejemplo: La regla \texttt{Block port 80} con protocolo ``\texttt{ANY}" genera tres reglas:

\begin{enumerate}
    \item \texttt{Block port 80} (\texttt{TCP})
    \item \texttt{Block port 80} (\texttt{UDP})
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Expandir reglas genéricas (\texttt{protocol:} ``\texttt{ANY}") en reglas específicas
    \item Inicializar estructuras de datos:
    \begin{itemize}
        \item \texttt{self.rules}: Lista de reglas de firewall expandidas
        \item \texttt{self.mac\_to\_port}: Diccionario para \texttt{MAC} learning por switch
    \end{itemize}
...
\textbf{Nota importante:} La constante \texttt{FIREWALL\_SWITCH} está definida en el código del controlador como:

\begin{lstlisting}[language=Python]
FIREWALL_SWITCH = "00-00-00-00-00-02"  # Switch s2
\end{lstlisting}

Esto garantiza que las reglas de firewall solo se instalen en el switch designado (\texttt{s2}), mientras que los demás switches (\texttt{s1}, \texttt{s3}, \texttt{s4}) operan únicamente con \texttt{MAC} learning y forwarding.
...
\end{minipage}
\end{center}
\caption{Aprendizaje \texttt{MAC} y forwarding de paquete permitido}
...
La función \texttt{install\_rule()} traduce una regla de firewall en formato \texttt{JSON} a un mensaje FlowMod de OpenFlow:
...
\begin{itemize}
    \item \textbf{Validación de reglas:} El archivo \texttt{JSON} se valida al cargar. Reglas inválidas se ignoran con warning en los logs.
    ...
    \item \textbf{Detección de duplicados:} El mecanismo de \texttt{MAC} learning actualiza la tabla solo cuando aprende nueva información, evitando instalaciones redundantes.
    
    \item \textbf{Logging multinivel:} El sistema registra eventos a nivel \texttt{INFO} (operaciones principales), \texttt{DEBUG} (detalles de matching) y \texttt{WARNING} (errores no críticos).
\end{itemize}
...
Para modo verbose con logging \texttt{DEBUG}:
...
\begin{description}
    ...
    \item[\texttt{--mac}] Asigna \texttt{MAC}s secuenciales (\texttt{00:00:00:00:00:01}, ...)
    \item[\texttt{--arp}] Pre-popula tabla \texttt{ARP} para evitar broadcasts
\end{description}

\subsubsection{Verificar Flows Instalados}

Para inspeccionar las reglas activas en un switch usando \texttt{ovs-ofctl}\cite{ovs_ofctl}:
...
\end{Sbox}
\fbox{\TheSbox}
\caption{Flows instalados en el switch s2}
\label{lab:flows_instalados}
\end{log}
...
Para analizar el tráfico OpenFlow entre controlador y switch usando Wireshark\cite{wireshark}:
...
\begin{itemize}
    ...
    \item \texttt{tcp.port == 80}: Ver tráfico \texttt{HTTP} bloqueado
    \item \texttt{udp.port == 5001}: Ver tráfico \texttt{UDP} específico
...
\end{Sbox}
\fbox{\TheSbox}
\caption{Logs de inicialización e instalación de reglas}
\label{lab:logs_init}
\end{log}

Para debugging avanzado, activar nivel \texttt{DEBUG}:
...
\begin{itemize}
    ...
    \item Decisiones de forwarding y \texttt{MAC} learning
